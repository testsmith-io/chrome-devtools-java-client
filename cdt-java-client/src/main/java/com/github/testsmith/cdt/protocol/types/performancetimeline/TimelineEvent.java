package com.github.testsmith.cdt.protocol.types.performancetimeline;

/*-
 * #%L
 * cdt-java-client
 * %%
 * Copyright (C) 2018 - 2024 Kenan Klisura
 * %%
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 * #L%
 */

import com.github.testsmith.cdt.protocol.support.annotations.Optional;

/**
 * TimelineEvent class.
 *
 * @author roy
 * @version $Id: $Id
 */
public class TimelineEvent {

  private String frameId;

  private String type;

  private String name;

  private Double time;

  @Optional private Double duration;

  @Optional private LargestContentfulPaint lcpDetails;

  @Optional private LayoutShift layoutShiftDetails;

  /**
   * Identifies the frame that this event is related to. Empty for non-frame targets.
   *
   * @return a {@link java.lang.String} object
   */
  public String getFrameId() {
    return frameId;
  }

  /**
   * Identifies the frame that this event is related to. Empty for non-frame targets.
   *
   * @param frameId a {@link java.lang.String} object
   */
  public void setFrameId(String frameId) {
    this.frameId = frameId;
  }

  /**
   * The event type, as specified in
   * https://w3c.github.io/performance-timeline/#dom-performanceentry-entrytype This determines
   * which of the optional "details" fiedls is present.
   *
   * @return a {@link java.lang.String} object
   */
  public String getType() {
    return type;
  }

  /**
   * The event type, as specified in
   * https://w3c.github.io/performance-timeline/#dom-performanceentry-entrytype This determines
   * which of the optional "details" fiedls is present.
   *
   * @param type a {@link java.lang.String} object
   */
  public void setType(String type) {
    this.type = type;
  }

  /**
   * Name may be empty depending on the type.
   *
   * @return a {@link java.lang.String} object
   */
  public String getName() {
    return name;
  }

  /**
   * Name may be empty depending on the type.
   *
   * @param name a {@link java.lang.String} object
   */
  public void setName(String name) {
    this.name = name;
  }

  /**
   * Time in seconds since Epoch, monotonically increasing within document lifetime.
   *
   * @return a {@link java.lang.Double} object
   */
  public Double getTime() {
    return time;
  }

  /**
   * Time in seconds since Epoch, monotonically increasing within document lifetime.
   *
   * @param time a {@link java.lang.Double} object
   */
  public void setTime(Double time) {
    this.time = time;
  }

  /**
   * Event duration, if applicable.
   *
   * @return a {@link java.lang.Double} object
   */
  public Double getDuration() {
    return duration;
  }

  /**
   * Event duration, if applicable.
   *
   * @param duration a {@link java.lang.Double} object
   */
  public void setDuration(Double duration) {
    this.duration = duration;
  }

  /**
   * Getter for the field <code>lcpDetails</code>.
   *
   * @return a {@link
   *     com.github.testsmith.cdt.protocol.types.performancetimeline.LargestContentfulPaint} object
   */
  public LargestContentfulPaint getLcpDetails() {
    return lcpDetails;
  }

  /**
   * Setter for the field <code>lcpDetails</code>.
   *
   * @param lcpDetails a {@link
   *     com.github.testsmith.cdt.protocol.types.performancetimeline.LargestContentfulPaint} object
   */
  public void setLcpDetails(LargestContentfulPaint lcpDetails) {
    this.lcpDetails = lcpDetails;
  }

  /**
   * Getter for the field <code>layoutShiftDetails</code>.
   *
   * @return a {@link com.github.testsmith.cdt.protocol.types.performancetimeline.LayoutShift}
   *     object
   */
  public LayoutShift getLayoutShiftDetails() {
    return layoutShiftDetails;
  }

  /**
   * Setter for the field <code>layoutShiftDetails</code>.
   *
   * @param layoutShiftDetails a {@link
   *     com.github.testsmith.cdt.protocol.types.performancetimeline.LayoutShift} object
   */
  public void setLayoutShiftDetails(LayoutShift layoutShiftDetails) {
    this.layoutShiftDetails = layoutShiftDetails;
  }
}
